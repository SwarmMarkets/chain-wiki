 ...
 * ```
 * import { getContractEvents } from "thirdweb";
 * import { approvedEvent } from "TODO";
 *
 * const events = await getContractEvents({
 * contract,
 * events: [
 ...
 * @param options - The options for the isApprovedFor function.
 * @returns The parsed result of the function call.
 * @example
 * ```
 * import { isApprovedFor } from "TODO";
 *
 * const result = await isApprovedFor({
 *  owner: ...,
 *  node: ...,
 *  delegate: ...,
 * });
 ...
 * ```
 */
export async function isApprovedFor(
  options: BaseTransactionOptions<IsApprovedForParams>
) {
  return readContract({
    contract: options.contract,
    method: [
